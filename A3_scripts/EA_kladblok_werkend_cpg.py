...
# NEW: EvoTorch for the inner-loop CMA-ES
import torch
...
# ---------- Controller (kept for video/demo fallback) ----------
...
SMOOTH_ALPHA         = 0.12  # smoothing toward target
...
def optimize_cpg_cma_for_body(model: mj.MjModel, seconds: float = 6.0, pop: int = 40, gens: int = 20) -> np.ndarray:
    ...
    print("best_theta[:5] (head):", best_theta[:5])

    for _ in range(int(gens)):
        solver.step()
        pop = solver.population

        # SAFE reads: read-only views (do NOT call access_values without keep_evals=True)
        vals_t = pop.values           # shape: (pop, L)
        evals_t = pop.evals           # shape: (pop, 1) for single-objective

        vals = vals_t.detach().cpu().numpy()
        fits = evals_t.detach().cpu().numpy().reshape(-1)  # (pop,)

        # Ignore any accidental NaNs robustly
        finite = np.isfinite(fits)
        if np.any(finite):
            i = int(np.argmin(fits[finite]))
            # map back to original indices
            true_i = int(np.arange(len(fits))[finite][i])
            if fits[true_i] < best_eval:
                best_eval = float(fits[true_i])
                best_theta = vals[true_i].copy()

    # Extra safety: also query solver's own notion of "best"
    best_sol = solver.status.get("best")
    if best_sol is not None:
        # best_sol is a Solution; its values is a read-only tensor
        theta_from_status = best_sol.values.detach().cpu().numpy()
        # trust it if it’s better (or if we never updated)
        best_eval_status = solver.status.get("best_eval")
        if (best_eval_status is not None) and (best_eval_status <= best_eval or not np.isfinite(best_eval)):
            best_theta = theta_from_status.copy()
...
def evaluate(nde: NeuralDevelopmentalEncoding, genotype: list[np.ndarray]) -> tuple[float, "DiGraph"]:
    """Decode → TRAIN CPG (train spec) → EVAL with tracker (fresh spec) → fitness."""
    ...
    # -------- TRAIN copy (attach once, only for CMA-ES) --------
    core_train = construct_mjspec_from_graph(robot_graph)
    mj.set_mjcb_control(None)
    train_world = OlympicArena()
    train_world.spawn(core_train.spec, spawn_position=SPAWN_POS)
    train_model = train_world.spec.compile()

    theta_best = optimize_cpg_cma_for_body(train_model, seconds=6.0, pop=30, gens=15)

    # -------- EVAL copy (fresh spec for tracked evaluation) --------
    core_eval = construct_mjspec_from_graph(robot_graph)

    def cpg_callback(m: mj.MjModel, d: mj.MjData) -> npt.NDArray[np.float64]:
        cb = make_cpg_controller(theta_best, m)
        cb(m, d)
        return d.ctrl
    ...
    # IMPORTANT: do NOT reuse core_train; use the fresh core_eval
    # Also: no video recording during per-individual evaluation
    experiment(robot=core_eval, controller=ctrl, duration=6, record=False)
...
def main() -> None:
    ...
    # Save best fitness per generation (unchanged)
    # ...

    # -------- Make the video with the trained CPG on the best body --------
    # TRAIN copy just to get the best theta for the best_graph
    best_core_train = construct_mjspec_from_graph(best_graph)
    mj.set_mjcb_control(None)
    best_world = OlympicArena()
    best_world.spawn(best_core_train.spec, spawn_position=SPAWN_POS)
    best_model = best_world.spec.compile()

    theta_best = optimize_cpg_cma_for_body(best_model, seconds=6.0, pop=30, gens=15)

    # EVAL copy for the actual recorded run
    best_core_eval = construct_mjspec_from_graph(best_graph)

    def cpg_video_callback(m: mj.MjModel, d: mj.MjData) -> npt.NDArray[np.float64]:
        cb = make_cpg_controller(theta_best, m)
        cb(m, d)
        return d.ctrl

    tracker = Tracker(mujoco_obj_to_find=mj.mjtObj.mjOBJ_GEOM, name_to_bind="core")
    ctrl = Controller(controller_callback_function=cpg_video_callback, tracker=tracker)

    console.log("[yellow]Recording video of best robot (CPG)...[/yellow]")
    video_folder = DATA / "videos"
    video_folder.mkdir(exist_ok=True)
    experiment(robot=best_core_eval, controller=ctrl, duration=10, record=True)
